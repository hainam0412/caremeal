...
<?php 
	...
	if ( is_user_logged_in() ) { 
		...
		$crus = $current_user->user_login;

		echo '<!--' . $crus . '-->';

		global $wpdb;

		$results = $wpdb->get_results( "SELECT * FROM wp_users WHERE user_login = '$crus'", OBJECT);

		echo '<!--' . $results[0]->user_email . '-->';

		$results2 = $wpdb->get_results( "SELECT * FROM wp_usermeta WHERE user_id = '" . strval($results[0]->ID) . "' AND meta_key = 'student_height'", OBJECT);

		$results3 = $wpdb->get_results( "SELECT * FROM wp_usermeta WHERE user_id = '" . strval($results[0]->ID) . "' AND meta_key = 'student_weight'", OBJECT);

		echo 'Height: ' . $results2[0]->meta_value . '(m).';

		echo '<br>';

		echo 'Weight: ' . $results3[0]->meta_value . '(Kg).';

		echo '<br>';

		$bmi = round((($results3[0]->meta_value)/(($results2[0]->meta_value)*($results2[0]->meta_value))), 1);
		...
		if ($bmi < 16) {
			...
			echo 'Chỉ số BMI ở trên cho thấy bạn bị gầy độ III!';

		} else if (16 <= $bmi && $bmi <= 17) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị gầy độ II!';

		} else if (17 <= $bmi && $bmi <= 18.5) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị gầy độ I!';

		} else if (18.5 <= $bmi && $bmi <= 25) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bình thường!';

		} else if (25 <= $bmi && $bmi <= 30) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị thừa cân!';

		} else if (30 <= $bmi && $bmi <= 35) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị béo phì độ I!';

		} else if (35 <= $bmi && $bmi <= 40) {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị béo phì độ II!';

		} else {

			echo 'Chỉ số BMI ở trên cho thấy bạn bị béo phì độ III';

		}
		echo '<br>';
		echo '<br>';
		echo '<a id="vb-add_dl" href="#">Thêm dữ liệu món ăn hàng ngày</a>';
	} 
...
?>
		...
		<?php echo do_shortcode('[thong_ke]'); ?>
